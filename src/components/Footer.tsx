...
export default function Footer({
  ...
  return (
    ...
    <footer className="bg-brandsec mt-auto border-t border-gray-200">
      <div className="mx-auto max-w-7xl overflow-hidden px-6 py-12 lg:px-8">
        ...
        >
          ...
          {showServicesSection && (
            <div className="pb-6">
              <Link
                href={leistungenPath}
                className={`text-sm leading-6 ${linkColorClass} transition-colors`}
              >
                Leistungen
              </Link>
            </div>
        ...
        <p className={`mt-10 text-center text-xs leading-5 ${mutedColorClass}`}>
          {" "}
          &copy; {new Date().getFullYear()} {businessName || "Ihr Firmenname"}.
          Alle Rechte vorbehalten.{" "}
        </p>
        <p className={`mt-2 text-center text-xs leading-5 ${mutedColorClass}`}>
          {" "}
