...
export default function DashboardProgress() {
  ...
  const hasLegalTexts = !!(profile.impressum_text && profile.datenschutz_text && profile.impressum_text.length > 10);
  ...
  return (
    <div className="rounded-2xl border border-orange-100 bg-white p-6 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        <div>
          <h3 className="font-semibold text-slate-900">Ihr Einrichtungs-Fortschritt</h3>
          <p className="text-sm text-slate-500">Nur noch wenige Schritte bis zur perfekten Seite.</p>
        </div>
        <span className="text-2xl font-bold text-orange-500">{progress}%</span>
      </div>
      
      {/* Progress Bar */}
      <div className="h-2.5 w-full rounded-full bg-slate-100 mb-5 overflow-hidden">
        <div 
      ...
      {nextStep && (
        <div className="flex items-center justify-between rounded-xl bg-orange-50 px-4 py-3">
          <span className="text-sm font-medium text-orange-800">
            Nächster Schritt: <span className="font-bold">{nextStep.label}</span>
          </span>
          <Link 
            href={nextStep.href}
