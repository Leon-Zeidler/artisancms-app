...
const serviceIcons = {
  Sanitärinstallation:
    "M13.5 10.5V6.75a4.5 4.5 0 119 0v3.75M3.75 10.5V6.75a4.5 4.5 0 10-9 0v3.75M3.75 12.75h16.5m-16.5 3h16.5",
  Heizungstechnik: "M12 21v-8.25M15.75 21v-8.25M8.25 21v-8.25M3 9l9-6 9 6",
  Fliesenarbeiten:
    "M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5m8.25 3v6.75m0 0l-3-3m3 3l3-3M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125V6.375c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.125c0 .621.504 1.125 1.125 1.125z",
  Default:
    "M4.5 12a7.5 7.5 0 0015 0m-15 0a7.5 7.5 0 1115 0m-15 0H3m18 0h-1.5m-15.75 0h.008v.008H4.25v-.008z",
};
...
export default function ClientHomepage() {
  ...
  const parsedServices =
    (profile.services_description
      ...
      .map((line: string) => { // <-- FIX: 'line: any' zu 'line: string'
        ...
        if (name && description) {
          const iconKey =
            Object.keys(serviceIcons).find((key) =>
              name.toLowerCase().includes(key.toLowerCase()),
            ) || "Default";
          return {
            name,
            description,
            icon: serviceIcons[iconKey as keyof typeof serviceIcons],
          };
        }
        return null;
      })
  ...
  const servicesSectionTextColor = isServicesDark
    ? "text-white"
    : "text-gray-900";
    ...
    <>
        ...
        <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col gap-16 lg:flex-row lg:items-center lg:gap-24">
          ...
          <div className="card-surface flex-1 rounded-2xl bg-white/80 p-8 text-center shadow-xl backdrop-blur-md sm:p-10 lg:text-left">
            ...
            {heroHighlights.length > 0 && (
              <ul className="mt-8 flex flex-col gap-3 text-sm text-gray-700 sm:flex-row sm:flex-wrap">
                {heroHighlights.map((service) => (
                  <li
                    key={service.name}
                    className="shadow-brand/10 inline-flex items-center gap-2 rounded-full bg-white/80 px-4 py-2 shadow-sm"
                  >
                    <span className="bg-brand/10 text-brand inline-flex size-6 items-center justify-center rounded-full">
                      <Icon path={service.icon} className="size-4" />
                    </span>
            ...
            <div className="mt-10 flex flex-col items-center gap-4 sm:flex-row sm:justify-start">
              <Link
                href="#kontakt"
                className="bg-brand shadow-brand/20 hover:bg-brand-dark focus-visible:outline-brand flex w-full items-center justify-center rounded-xl px-6 py-3 text-base font-semibold text-white shadow-lg transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 sm:w-auto"
              >
                Angebot anfordern
              </Link>
              {profile.slug && (
                <Link
                  ...
                  className="hover:text-brand flex items-center justify-center text-base font-semibold text-gray-900 transition"
                >
          ...
          <div className="flex-1">
            <div className="card-surface mx-auto w-full max-w-md space-y-6 p-6 sm:max-w-lg sm:p-8">
              ...
              <p className="text-sm text-gray-600">
                Haben Sie Fragen zu einem laufenden Projekt oder möchten Sie ein
                unverbindliches Angebot? Wir sind nur einen Klick entfernt.
              </p>
              <div className="space-y-4 text-sm text-gray-700">
                {profile.address && (
                  <div className="rounded-2xl bg-white/60 p-4 shadow-sm">
                    <p className="font-semibold text-gray-900">Standort</p>
                    <address className="mt-1 whitespace-pre-line not-italic text-gray-600">
                      {profile.address}
                    </address>
                  </div>
                )}
                {profile.phone && (
                  <div className="rounded-2xl bg-white/60 p-4 shadow-sm">
                    ...
                    <a
                      ...
                      className="hover:text-brand mt-1 block text-gray-600 hover:underline"
                    >
                      {profile.phone}
                    </a>
                  </div>
                ...
                <div className="flex flex-wrap gap-3">
                  {profile.phone && (
                    <a
                      href={`tel:${profile.phone}`}
                      className="border-brand/30 bg-brand/10 text-brand hover:bg-brand/20 focus-visible:outline-brand inline-flex flex-1 items-center justify-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-semibold transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 sm:flex-none"
                    >
                  ...
                  {profile.email && (
                    <a
                      href={`mailto:${profile.email}`}
                      className="hover:border-brand hover:text-brand focus-visible:outline-brand inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 sm:flex-none"
                    >
          ...
          {/* ... (Rest der Services Section) */}
           <div className="mx-auto max-w-7xl px-6 lg:px-8">
            <div className="mx-auto max-w-2xl text-center lg:max-w-3xl">
              ...
              <p
                className={`mt-3 text-3xl font-bold tracking-tight sm:text-4xl ${servicesHeadingColor}`}
              >
                {industryTemplate.servicesSectionTitle}
              </p>
              <p className={`mt-3 text-sm lg:text-base ${servicesTextColor}`}>
                Passgenaue Lösungen für private und gewerbliche Projekte –
                sorgfältig geplant und zuverlässig umgesetzt.
              </p>
            </div>
        ...
        <section id="projekte" className="py-24 sm:py-32">
          ...
          <div className="mx-auto max-w-7xl px-6 lg:px-8">
            <div className="mx-auto max-w-2xl text-center">
              ...
              <p className="mt-6 text-base leading-7 text-gray-600 sm:text-lg sm:leading-8">
                Entdecken Sie Projekte, die wir für zufriedene Kundinnen und
                Kunden realisiert haben.
              </p>
            </div>
            <div className="mx-auto mt-16 grid max-w-2xl grid-cols-1 gap-10 lg:mx-0 lg:max-w-none lg:grid-cols-3">
              {featuredProjects.map((project) => {
                const projectDate =
                  project["project-date"] || project.created_at;
                ...
                return (
                  <article key={project.id} className="flex h-full flex-col">
                    <Link
                      href={`/${profile.slug}/portfolio/${project.id}`}
                      ...
                      <div className="card-surface flex h-full flex-col overflow-hidden transition duration-200 group-hover:-translate-y-1">
                        ...
                        <div className="flex flex-1 flex-col space-y-4 p-6">
                          <div className="flex items-center justify-between gap-3 text-xs text-gray-500">
                            {formattedDate && (
                              <time dateTime={projectDate || undefined}>
                                {formattedDate}
                              </time>
                            )}
                            <span
                          ...
                          <h3 className="group-hover:text-brand text-lg font-semibold text-gray-900 transition-colors">
                            {project.title || "Unbenanntes Projekt"}
                          </h3>
                          <p className="text-sm leading-6 text-gray-600">
                            {summary}
                          </p>
                          <span className="text-brand mt-auto inline-flex items-center gap-2 text-sm font-semibold">
                            Projekt ansehen
            ...
            <div className="mt-16 text-center">
              <Link
                href={`/${profile.slug}/portfolio`}
                className="bg-brand shadow-brand/20 hover:bg-brand-dark focus-visible:outline-brand rounded-xl px-5 py-3 text-base font-semibold text-white shadow-lg transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2"
              >
      ...
      {testimonials.length > 0 && (
        ...
        >
          ...
          />
          <div
            className="absolute inset-y-0 right-1/2 -z-10 mr-16 hidden w-[200%] origin-bottom-left skew-x-[-30deg] bg-white shadow-xl shadow-indigo-600/10 ring-1 ring-indigo-50 sm:mr-28 sm:block lg:mr-0 xl:mr-16 xl:origin-center"
            aria-hidden="true"
          />
            ...
            {profile.slug && (
              <div className="mt-16 text-center">
                <Link
                  ...
                  className="hover:text-brand text-base font-semibold leading-6 text-gray-900 transition-colors"
                >
          ...
          <div className="grid grid-cols-1 gap-x-12 gap-y-16 lg:grid-cols-5">
            <div className="space-y-6 lg:col-span-2">
              ...
              <p className="text-sm leading-6 text-gray-600 sm:text-base">
                Nutzen Sie das Formular oder greifen Sie direkt zum Hörer – wir
              ...
              <div className="space-y-5 text-sm leading-6 text-gray-600 sm:text-base">
                {profile.address && (
                  <div className="flex gap-x-4">
                    <svg
                      className="size-6 flex-none text-gray-500"
                      fill="none"
                      viewBox="0 0 24 24"
                      strokeWidth={1.5}
                      stroke="currentColor"
                    ...
                    <address className="whitespace-pre-line not-italic text-gray-600">
                      {profile.address}
                    </address>
                  </div>
                )}
            ...
            <div className="lg:col-span-3">
              <div className="card-surface p-6 sm:p-8">
                <div aria-live="polite" className="space-y-3">
                  {formStatus === "success" && (
                    <div
                      className="flex items-start gap-3 rounded-xl border border-green-200 bg-green-50/90 p-3 text-sm text-green-700"
                      role="status"
                    >
                      <CheckCircleIcon
                  ...
                  {formStatus === "error" && (
                    <div
                      className="flex items-start gap-3 rounded-xl border border-red-200 bg-red-50/80 p-3 text-sm text-red-700"
                      role="alert"
                    >
                      ...
                      <div>
                        ...
                        <p>
                          {formError || "Bitte versuchen Sie es später erneut."}
                        </p>
                      </div>
                    </div>
                  )}
                ...
                >
                  <div>
                    ...
                    <div className="mt-2.5">
                      <input
                        ...
                        className="focus:border-brand focus:ring-brand block w-full rounded-xl border border-slate-200/70 bg-white/90 px-4 py-3 text-sm text-gray-900 shadow-sm transition focus:outline-none focus:ring-2 sm:text-base"
                      />
                    </div>
                  </div>
                  <div>
                  ...
                  <div className="flex justify-end">
                    <button
                      type="submit"
                      disabled={formStatus === "loading"}
                      className="bg-brand shadow-brand/20 hover:bg-brand-dark focus-visible:outline-brand inline-flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-semibold text-white shadow-lg transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 disabled:cursor-not-allowed disabled:opacity-60 sm:text-base"
                    >
                      {formStatus === "loading" && (
