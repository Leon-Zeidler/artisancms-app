  ...
  useEffect(() => {
    ...
    const fetchTeamData = async () => {
      ...
      try {
        ...
        if (fetchError) {
          setError(
            `Team-Mitglieder konnten nicht geladen werden: ${fetchError.message}`,
          );
        } else {
          setTeamMembers(data || []);
        }
              ...
              teamMembers.map((person) => (
                <li key={person.id} className="flex flex-col gap-6">
                  {person.avatar_url ? (
                    ...
                    <div className="flex aspect-[3/2] w-full items-center justify-center rounded-2xl bg-slate-100">
                      <UserGroupIcon className="size-24 text-slate-400" />
                    </div>
                  )}
                  <div>
