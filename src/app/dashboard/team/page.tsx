...
type TeamMemberFormData = Omit<
  TeamMember,
  "id" | "created_at" | "user_id" | "profile_id"
>;
type ModalState = {
  isOpen: boolean;
  mode: "add" | "edit";
...
function TeamMemberModal({
  ...
  const [formData, setFormData] = useState<TeamMemberFormData>(
    modalState.mode === "edit" && modalState.data
      ? {
          ...
          bio: modalState.data.bio ?? "",
          display_order: modalState.data.display_order ?? 0,
          avatar_url: modalState.data.avatar_url,
          avatar_storage_path: modalState.data.avatar_storage_path,
        }
      : {
          name: "",
          role: "",
  ...
  const [avatarPreview, setAvatarPreview] = useState<string | null>(
    modalState.data?.avatar_url || null,
  );
  ...
  const handleChange = (
    ...
    setFormData((prev: TeamMemberFormData) => ({
      ...prev,
  ...
  return (
    <div
      ...
      role="dialog"
    >
      <div className="relative flex max-h-[90vh] w-full max-w-lg flex-col rounded-2xl border border-orange-100 bg-white p-6 shadow-2xl shadow-orange-100/50">
        <button
          onClick={onClose}
          disabled={isSaving}
          className="absolute right-4 top-4 text-slate-400 transition hover:text-orange-500 disabled:opacity-50"
        >
          {" "}
        ...
        >
          <div>
            ...
            <div className="flex items-center gap-4">
              <div className="flex size-24 shrink-0 items-center justify-center overflow-hidden rounded-full border border-slate-200 bg-white text-slate-400 shadow-sm">
                {avatarPreview ? (
              ...
              <div className="grow space-y-2">
                <input
                  ...
                  className="block w-full text-sm text-slate-500 file:mr-4 file:rounded-md file:border-0 file:bg-orange-100 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-orange-700 hover:file:bg-orange-200 focus:outline-none focus:ring-2 focus:ring-orange-200"
                />
                {avatarPreview && (
                  <button
                    type="button"
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className="w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm placeholder:text-slate-400 focus:border-orange-400 focus:outline-none focus:ring-2 focus:ring-orange-100"
              placeholder="z.B. Geschäftsführer, Meister, Geselle"
            />{" "}
          </div>
          <div>
            {" "}
            ...
            <textarea
              ...
              onChange={handleChange}
              className="w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm placeholder:text-slate-400 focus:border-orange-400 focus:outline-none focus:ring-2 focus:ring-orange-100"
              placeholder="z.B. Experte für Badsanierung seit 10 Jahren..."
            />{" "}
          </div>
          <div>
            {" "}
            ...
            <p className="mt-1 text-xs text-slate-500">
              Niedrigere Zahlen werden zuerst angezeigt (z.B. 0, 1, 2...).
            </p>{" "}
          </div>
        ...
        <div className="mt-4 flex justify-end gap-3 border-t border-orange-100 pt-6">
          <button
            ...
            onClick={onClose}
            disabled={isSaving}
            className="rounded-full px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-100 disabled:opacity-50"
          >
            {" "}
            Abbrechen{" "}
          </button>
          <button
            ...
            disabled={isSaving}
            className={`inline-flex items-center gap-x-2 rounded-full px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-orange-200 transition ${isSaving ? "cursor-not-allowed bg-orange-300" : "bg-orange-500 hover:bg-orange-400"}`}
          >
            {" "}
...
export default function TeamManagementPage() {
  ...
  const [actionLoading, setActionLoading] = useState<
    Record<string, "delete" | "save" | null>
  >({});
  ...
  const fetchTeamMembers = useCallback(
    async (user: User) => {
      ...
      if (fetchError) {
        ...
        setError(
          `Mitglieder konnten nicht geladen werden: ${fetchError.message}`,
        );
        setTeamMembers([]);
      } else {
        setTeamMembers(data || []);
  ...
  const openEditModal = (member: TeamMember) =>
    setModalState({ isOpen: true, mode: "edit", data: member });
  const closeModal = () => {
    if (actionLoading.modal === "save") return;
  ...
  const setLoadingState = (
    ...
    setActionLoading((prev: typeof actionLoading) => ({ ...prev, [id]: type }));
  };
  ...
  const handleSaveMember = async (
    ...
    const dataToUpsert: Omit<TeamMember, "id" | "created_at"> = {
      ...formData,
    ...
    try {
      if (isRemovingAvatar && id && formData.avatar_storage_path) {
        await supabase.storage
          .from("team-avatars")
          .remove([formData.avatar_storage_path]);
  ...
  const handleDeleteRequest = (member: TeamMember) => {
    setDeleteConfirmState({ isOpen: true, member: member });
  };
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (memberToDelete.avatar_storage_path) {
        await supabase.storage
          .from("team-avatars")
          .remove([memberToDelete.avatar_storage_path]);
      }

      toast.success("Mitglied gelöscht!", { id: toastId });
      setTeamMembers((prev: TeamMember[]) =>
        prev.filter((t: TeamMember) => t.id !== memberToDelete.id),
      );
    } catch (err: any) {
  ...
  const distinctRoles = new Set(
    teamMembers.map((member) => member.role).filter(Boolean),
  ).size;
  const membersWithBio = teamMembers.filter(
    (member) => (member.bio ?? "").trim().length > 0,
  ).length;
  const bioCoverage = totalMembers
    ? Math.round((membersWithBio / totalMembers) * 100)
    : 0;
  ...
  return (
    <main className="space-y-10 px-6 py-10 lg:px-10">
      ...
      >
        <div className="grid gap-4 md:grid-cols-3">
          <DashboardStatCard
            ...
            trend={
              totalMembers > 0
                ? `${totalMembers} Personen sichtbar`
                : "Fügen Sie Ihr erstes Mitglied hinzu"
            }
          />
          <DashboardStatCard
          ...
          <div className="space-y-4">
            {teamMembers.length > 0
              ? teamMembers.map((member: TeamMember) => {
                  ...
                  const isDisabled =
                    isLoading ||
                    actionLoading.modal === "save" ||
                    (deleteConfirmState.member?.id === member.id &&
                      isConfirmingDelete);
                  return (
                    <div
                      key={member.id}
                      className={`flex items-center justify-between rounded-2xl border border-orange-100 bg-white/90 p-4 shadow-sm shadow-orange-100 transition hover:-translate-y-0.5 hover:border-orange-300 hover:shadow-lg ${isDisabled ? "pointer-events-none opacity-70" : ""}`}
                    >
          ...
          <div className="space-y-4">
            <div className="overflow-hidden rounded-2xl border border-orange-100 bg-white/90 shadow-lg shadow-orange-100/40">
              ...
              <div className="space-y-3 px-5 py-4 text-sm text-slate-600">
                <p>
                  Schreiben Sie kurze Bios, die Fachgebiete und persönliche
                  Highlights kombinieren – so wirkt Ihr Team nahbar.
                </p>
                <p>
                  Nutzen Sie hochwertige Porträts oder Arbeitsfotos mit
                  identischem Hintergrund, um einen einheitlichen Eindruck zu
                  schaffen.
                </p>
                <p>
                  Positionieren Sie Ihr wichtigstes Teammitglied an erster
                  Stelle durch die Sortier-Reihenfolge.
                </p>
              </div>
            </div>
            <div className="rounded-2xl border border-orange-100 bg-white/90 p-5 text-sm text-slate-600 shadow-lg shadow-orange-100/40">
              ...
              <p className="mt-2 text-sm text-slate-600">
                Ergänzen Sie Ihre Teamseite mit Testimonials und Projektlinks,
                damit Besucher direkt sehen, welche Qualität sie erwartet.
              </p>
              <button
      ...
      <TeamMemberModal
        ...
        isSaving={actionLoading.modal === "save"}
      />
