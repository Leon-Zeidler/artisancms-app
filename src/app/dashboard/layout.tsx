...
type SidebarLinkProps = {
  ...
  href: string;
  active?: boolean;
  isExternal?: boolean; // Keep this
};
// *** NEW: Profile type including slug ***
type Profile = {
    id: string;
    slug: string | null;
    ...
    <aside className="w-64 flex-shrink-0 bg-slate-800 p-4 relative">
      ...
      <nav className="space-y-2">
        ...
        <SidebarLink icon={SettingsIcon} text="Einstellungen" href="/dashboard/einstellungen" active={pathname === '/dashboard/einstellungen'} />
         {/* Link to Public Website - Now uses dynamic websiteHref */}
         <div className="pt-4 mt-4 border-t border-slate-700">
             <SidebarLink
                icon={ArrowTopRightOnSquareIcon}
                text="Meine Webseite"
                href={websiteHref} // Use the dynamic href
                isExternal={true}
             />
         </div>
      </nav>
...
export default function DashboardLayout({
  ...
  const [userSlug, setUserSlug] = useState<string | null>(null); // *** NEW: State for slug ***
