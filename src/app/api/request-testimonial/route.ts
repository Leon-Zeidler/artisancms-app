...
export async function POST(request: Request) {
  ...
  if (!customerName || !customerEmail || !projectName) {
    return NextResponse.json({ error: 'Fehlende Felder: customerName, customerEmail und projectName sind erforderlich' }, { status: 400 });
  }
  ...
  try {
    ...
    console.warn(`Konnte Profil für User ${user.id} nicht laden:`, profileError);
  ...
  if (insertError) {
    console.error('Fehler beim Speichern der Testimonial-Anfrage:', insertError);
  ...
  let emailSubject = `Wie zufrieden waren Sie mit ${businessName}?`;
  let emailHtmlBody = `
