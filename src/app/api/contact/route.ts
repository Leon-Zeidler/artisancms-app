...
export async function POST(request: Request) {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    console.error(
      "[API Contact - POST] FATAL: Supabase service configuration missing.",
    );
    return NextResponse.json(
      { error: "Server configuration error: Database service key missing." },
      { status: 500 },
    );
  }
  ...
  if (!anonKey) {
    console.error(
      "[API Contact - POST] FATAL: Supabase anon configuration missing.",
    );
    return NextResponse.json(
      { error: "Server configuration error: Database anon key missing." },
      { status: 500 },
    );
  }
  ...
  if (!resendApiKey) {
    console.error(
      "[API Contact - POST] FATAL: RESEND_API_KEY missing at runtime.",
    );
    return NextResponse.json(
      { error: "Server configuration error: Email service key missing." },
      { status: 500 },
    );
  }
  ...
  if (!name || !senderEmail || !message || !profileId) {
    return NextResponse.json(
      { error: "Name, email, message, and profileId are required" },
      { status: 400 },
    );
  }
  if (
  ...
  try {
    ...
    if (userError) {
      ...
      return NextResponse.json(
        { error: "Could not identify recipient auth." },
        { status: 500 },
      );
    }
    if (profileError) {
      ...
      return NextResponse.json(
        { error: "Could not identify recipient profile." },
        { status: 500 },
      );
    }
    ...
    const recipientBusinessName =
      profileData?.business_name || "Ihrer Webseite";
    ...
    if (emailError) {
      ...
      return NextResponse.json(
        { error: "Failed to send message email notification." },
        { status: 500 },
      );
    }
    console.log(
      "Email sent successfully via Resend request ID:",
      emailData?.id,
    );
