...
export async function POST(request: Request) {
  try {
    ...
    if (!token || !author_name || !body) {
      return NextResponse.json({ error: 'Missing required fields: token, author_name, body' }, { status: 400 });
    }
    ...
    if (requestData.status !== 'pending') {
      return NextResponse.json({ error: 'Review link has expired or already been used.' }, { status: 410 });
    }
    ...
    if (updateError) {
      ...
      console.error("Warning: Could not mark request as completed:", updateError.message);
    }
