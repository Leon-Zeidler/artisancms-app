  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        console.log(`Checkout complete for user ${userId}. Plan: ${planId}. Updating profile...`);
        ...
        console.log(`User ${userId} successfully updated to active '${planId}' subscription.`);
        break;
      }

      case 'customer.subscription.deleted': {
        const subscription = event.data.object as Stripe.Subscription;
        console.log(`Subscription ${subscription.id} deleted. Updating profile...`);
        ...
        if (error) {
            throw new Error(`Supabase DB update for deleted subscription failed: ${error.message}`);
        }
        break;
      }
      case 'customer.subscription.updated': {
        ...
        const updateData: { subscription_status: string; plan_id?: PlanId | null } = {
        ...
        if (error) {
          throw new Error(`Supabase DB update for subscription update failed: ${error.message}`);
        }
        break;
      }
    }
