...
export async function POST(request: Request) {
  ...
  try {
    ...
    const template =
      INDUSTRY_TEMPLATES[resolvedIndustry] ?? INDUSTRY_TEMPLATES.sonstiges;
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `OpenAI API request failed: ${errorData.error?.message || response.statusText}`,
      );
    }
    ...
    if (errorMessage.includes("Unauthorized")) {
      return NextResponse.json(
        {
          error:
            "OpenAI API key is invalid or missing. Please check server configuration.",
        },
        { status: 500 },
      );
    }
