...
import { INDUSTRY_TEMPLATES, resolveIndustry, type Industry } from '@/lib/industry-templates';
...
export async function POST(request: Request) {
  ...
  if (!title) {
    return NextResponse.json({ error: "Projekttitel ist erforderlich." }, { status: 400 });
  }
  ...
  try {
    ...
    console.warn("Could not fetch user profile for AI prompt, proceeding with defaults.");
  }
  ...
  try {
    ...
    if (!description) throw new Error("Failed to generate description from API response.");

    return NextResponse.json({ description });

  } catch (error: any) {
    console.error("Error calling OpenAI API:", error.message);
    return NextResponse.json({ error: `Failed to generate description: ${error.message}` }, { status: 500 });
  }
}